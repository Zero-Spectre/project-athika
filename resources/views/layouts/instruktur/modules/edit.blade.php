...
<div class="row">
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h1 class="mb-0">Edit Module "{{ $modul->judul }}"</h1>
    ...
    <div class="col-md-8 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                ...
                <form action="{{ route('instruktur.modules.update', $modul) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" id="tipe_materi" name="tipe_materi" required>
                            ...
                            <option value="Video" {{ old('tipe_materi', $modul->tipe_materi) == 'Video' ? 'selected' : '' }}>Video</option>
                            <option value="Artikel" {{ old('tipe_materi', $modul->tipe_materi) == 'Artikel' ? 'selected' : '' }}>Article</option>
                            <option value="Kuis" {{ old('tipe_materi', $modul->tipe_materi) == 'Kuis' ? 'selected' : '' }}>Quiz</option>
                        </select>
                    </div>
                    
                    <div class="mb-3">
                        <label for="urutan" class="form-label">Order</label>
                        <input type="number" class="form-control" id="urutan" name="urutan" value="{{ old('urutan', $modul->urutan) }}" min="1" required>
                        <div class="form-text">The order in which this module appears in the course.</div>
                    </div>
                    
                    <div class="mb-3" id="quiz_field" @if($modul->tipe_materi != 'Kuis') style="display: none;" @endif>
                        ...
                        <select class="form-select" id="quiz_id" name="quiz_id">
                            ...
                            @foreach($quizzes as $quiz)
                                <option value="{{ $quiz->id }}" {{ old('quiz_id', $modul->quiz_id) == $quiz->id ? 'selected' : '' }}>
                    ...
                    <div class="mb-3" id="konten_teks_field" @if($modul->tipe_materi != 'Artikel') style="display: none;" @endif>
                        <label for="konten_teks" class="form-label">Article Content</label>
                        <input id="konten_teks" type="hidden" name="konten_teks" value="{{ old('konten_teks', $modul->konten_teks) }}">
                        <trix-editor input="konten_teks"></trix-editor>
                    </div>
                    
                    <div class="mb-3" id="penjelasan_field" @if(!in_array($modul->tipe_materi, ['Artikel', 'Video'])) style="display: none;" @endif>
                        <label for="penjelasan" class="form-label">Detailed Explanation</label>
                        <textarea class="form-control" id="penjelasan" name="penjelasan" rows="5">{{ old('penjelasan', $modul->penjelasan) }}</textarea>
                        <div class="form-text">Provide a detailed explanation to help students understand the material.</div>
                    </div>
                    
                    <div class="mb-3" id="video_fields" @if($modul->tipe_materi != 'Video') style="display: none;" @endif>
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="video_type" id="video_type_upload" value="upload" {{ $modul->url_video ? '' : 'checked' }}>
                                <label class="form-check-label" for="video_type_upload">
                                    Upload Video
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="video_type" id="video_type_link" value="link" {{ $modul->url_video ? 'checked' : '' }}>
                        ...
                        <!-- Upload Video Fields -->
                        <div id="upload_video_fields" {{ $modul->url_video ? 'style=display:none;' : '' }}>
                            <div class="mb-3">
                                ...
                                @if($modul->url_video && strpos($modul->url_video, 'storage') !== false)
                                    <div class="mt-2">
                                        <small class="text-muted">Current video: {{ basename($modul->url_video) }}</small>
                                    </div>
                                @endif
                            </div>
                        </div>
                        ...
                        <!-- YouTube Link Fields -->
                        <div id="link_video_fields" {{ $modul->url_video && strpos($modul->url_video, 'youtube') !== false ? '' : 'style=display:none;' }}>
                            <div class="mb-3">
                                <label for="url_video" class="form-label">YouTube Video URL</label>
                                <input type="url" class="form-control" id="url_video" name="url_video" value="{{ old('url_video', $modul->url_video) }}" placeholder="https://www.youtube.com/watch?v=...">
                        ...
                        <div class="mb-3">
                            ...
                            @if($modul->video_thumbnail)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $modul->video_thumbnail) }}" alt="Current Thumbnail" class="img-thumbnail" style="max-height: 150px;">
                                </div>
                            @endif
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="video_deskripsi" name="video_deskripsi" rows="3">{{ old('video_deskripsi', $modul->video_deskripsi) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control" id="durasi_video" name="durasi_video" value="{{ old('durasi_video', $modul->durasi_video) }}" min="0">
                            <div class="form-text">Duration of the video in seconds.</div>
                        </div>
                    </div>
                    
                    <div class="d-flex justify-content-between">
                        <a href="{{ route('instruktur.courses.show', $modul->kursus) }}" class="btn btn-secondary">Cancel</a>
